...
public class UserController {
	...
	public ResponseData updateUser(@RequestBody UserDto userDto) {
		...
		User user1= userService.findUserById(userDto.getId());
		if(exist) {
			user1 =  userService.addUser(userService.userAssembler(userDto));
			ResponseData responseData= new ResponseData(HttpStatus.ACCEPTED.value(), "User Updated", user1);
			return responseData;
	     }
		else{
			user1 = userService.addUser(userService.userAssembler(userDto));
			ResponseData responseData= new ResponseData(HttpStatus.CREATED.value(), "User added", user1);
			return responseData;
		}	
	}
